"use client";

import { usePathname } from "next/navigation";

export default function Footer() {
    const pathname = usePathname();
    const isHome = pathname === "/";

    return (
    <footer className={`w-full h-[64px] flex items-center justify-between px-14 ${isHome ? "bg-[rgba(0,0,0,0.23)]" : "bg-[#2C2C2C]"}`}>

      {/* BACK */}
      <a href="#" className="flex items-center justify-center w-[75px] h-[32px] rounded-[31px] border-4 border-[#F4F4ED] bg-transparent">
        <span className="font-semibold text-[12px] text-[#F4F4ED]">BACK</span>
      </a>

      {/* TOP */}
      <button className="flex items-center justify-center w-[120px] h-[40px] rounded-[31px] border-4 border-[#F4F4ED] bg-transparent">
        <span className="font-semibold text-[14px] text-[#F4F4ED]">TOP</span>
      </button>

      {/* NEXT */}
      <a href="#" className="flex items-center justify-center w-[75px] h-[32px] rounded-[31px] border-4 border-[#F4F4ED] bg-transparent">
        <span className="font-semibold text-[12px] text-[#F4F4ED]">NEXT</span>
      </a>

    </footer>
  );
}
